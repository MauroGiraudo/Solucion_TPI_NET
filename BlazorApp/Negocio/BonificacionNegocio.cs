...
namespace BlazorApp.Negocio
{
    public class BonificacionNegocio
    {
        ...
        public static async Task<Bonificacion>? GetOne(int IdBonificacion)
        {
            var response = await Conexion.Instancia.Cliente.GetStringAsync(defaultURL + IdBonificacion);
            var data = JsonConvert.DeserializeObject<Bonificacion>(response);
            return data;
        }
