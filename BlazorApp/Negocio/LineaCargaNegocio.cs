...
namespace BlazorApp.Negocio
{
    public class LineaCargaNegocio
    {
        ...
        public async static Task<PrendaPedido> SetOnePrendaPedido(LineaCarga lineaCarga)
        {
            var prenda = await PrendaNegocio.GetOne(lineaCarga.IdPrenda);
            var precio = await PrecioPrendaNegocio.GetCurrentPrecio(lineaCarga.IdPrenda);
            PrendaPedido prendaPedido = new PrendaPedido
            {
                NumeroLinea = lineaCarga.NumeroLinea,
                Prenda = prenda.Descripcion,
                Cantidad = lineaCarga.CantidadPrenda,
                Precio = precio.Valor * lineaCarga.CantidadPrenda
            };
            return prendaPedido;
